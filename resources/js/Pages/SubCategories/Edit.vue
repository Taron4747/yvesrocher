<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/sub-categories">sub categories</Link>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          ...
          <loading-button :loading="form.processing" class="btn-indigo ml-auto" type="submit">Update sub category</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    update() {
      this.form.put(`/sub-categories/${this.category.id}`)
    },
    destroy() {
      if (confirm('Are you sure you want to delete this sub category?')) {
        this.$inertia.delete(`/sub-categories/${this.category.id}`)
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this sub  category?')) {
        this.$inertia.put(`/sub-categories/${this.category.id}/restore`)
      }
    },
  },
}
