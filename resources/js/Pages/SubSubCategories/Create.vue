    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="store">
        <div class="-mb-8 -mr-6 p-8">
          <div class="title_big">Подкатегория</div>
          <select-input v-model="form.parent_id" :error="form.errors.parent_id" class="pb-8 pr-6 w-full lg:w-1/2" label="Подкатегории">
            <option :value="null" />
        ...
        <div class="flex items-center justify-end px-8 py-4 bg-gray-50 border-t border-gray-100">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Создать Под Подкатегорию</loading-button>
        </div>
      </form>
    </div>
  </div>
